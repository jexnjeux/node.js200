...
app.get('/create', (request, repsonse) => {
    fs.readFile('./part5/insertNewBook.html', 'utf-8', (error, data) => {
        if (error) throw error;
        response.send(data);
    })
})
...
app.get('/modify/:id', (request, response) => {
    fs.readFile('./part5/modify.html', 'utf-8', (error, data)=> {
        connection.query('SELECT * from books WHERE number =?', [
            request.params.id
        ], (error, results) => {
            if (error) throw error;
